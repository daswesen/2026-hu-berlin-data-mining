...
# Block 1: Konzepte klären

## Ziele dieser Konsultation

- Offene Fragen zu den Materialien klären
- Die Begriffe sauber sortieren: KI, ML, Data Science, Data Mining
- Book-Crossing: vom rohen Datensatz zur Buchempfehlung

## Ablauf heute

| Block | Inhalt |
|-------|--------------------------------------------------|
| 1     | Begriffe, KI-Geschichte, CRISP-DM                |
| Pause |                                                  |
| 2     | Book-Crossing: Datenqualität & Apriori           |
...
- **KI**: Oberbegriff — Maschinen tun Dinge, die Intelligenz erfordern
- **Machine Learning**: Teilmenge der KI — Lernen aus Daten statt fester Regeln
- **Data Mining**: Mustersuche in (strukturierten) Daten — der Prozess
- **Text Mining**: dasselbe für unstrukturierte Texte
- **Data Science**: die Disziplin drumherum — inkl. Business Understanding, Kommunikation, Deployment
...
- 1950er/60er: erster Frühling — Turing, Dartmouth, große Versprechen
- 1970er: erster Winter — die Versprechen platzen
- 1980er: Expertensysteme — und der zweite Winter
- Heute: dritter Frühling?
- **Woran würden Sie den nächsten Winter erkennen?**

## Transfer in die Bibliothekswelt

- Welche KI-Versprechen hören Bibliotheken aktuell von Anbietern?
- KI-gestützte Erschließung, Chatbots, Discovery




## Der Book-Crossing-Datensatz

2004 von Cai-Nicolas Ziegler aus der BookCrossing-Community gecrawlt — die
...
## Übung: Was stimmt mit diesen Daten nicht? (15 Min)

- Gruppe 1: BX-Users
- Gruppe 2: BX-Books
- Gruppe 3: BX-Book-Ratings


## Auflösung: die üblichen Verdächtigen

- **Ratings:** 0 ist kein Verriss, sondern **implizit** — über die Hälfte aller Einträge!
...
## Ein Diagramm entlarvt die Daten

![](images/alter_histogramm.png)

- Altersangaben von 0 bis 244 — beim Alter wird gelogen (oder getippt)

## Exkurs: Was heißt Sparsity?

- Denkbild: eine Tabelle mit einer Zeile pro Nutzer:in und einer Spalte pro Buch, in den Zellen die Bewertungen
...
- Selbst populäre Buchpaare tauchen nur in einem Promille der Körbe auf — Schwellenwerte müssen winzig sein
...
- Werk vs. Manifestation → FRBR
- Uneinheitliche Namen → Normdaten
- Mehrfache ISBNs → Dublettenkontrolle
- **Bibliothekar:innen haben hier Fachkompetenz, die Data-Science-Teams fehlt**
- → Rolle: Subject Matter Expert

## Zur Erinnerung: FRBR (Sie kennen das besser als ich)

- IFLA-Referenzmodell (1998, heute Teil des IFLA LRM): **Werk → Expression → Manifestation → Exemplar**
- Am Beispiel: *The Green Mile* (Werk) → deutsche Übersetzung (Expression) → Signet-Taschenbuch mit eigener ISBN (Manifestation) → das Buch im Regal (Exemplar)
- Book-Crossing kennt **nur ISBNs = Manifestationen** — das Werk müssen wir per Data Mining rekonstruieren
- Genau das macht gleich das Record Linkage
...
## Vom Supermarkt zu Book-Crossing
...
## Von Hand: 8 Leser:innen

Jede Zeile ist ein:e Leser:in mit ihrem „Bücherregal" — wie 8 Kassenbons an der Supermarktkasse:
...
- Wir zählen einfach: In wie vielen der 8 Körbe liegen *Angels & Demons* **und** *Da Vinci Code* zusammen?
...
- Jetzt schauen wir nur in die Körbe mit *Angels & Demons* — das sind 4
...
- Der *Da Vinci Code* liegt sowieso in 6 von 8 Körben — 75 %, ein Bestseller
...
- Der Trick (Agrawal et al., 1994): **Was selten ist, fliegt früh raus** — samt aller Körbe und Kombinationen, in denen es steckt
...
- `r format(nrow(ratings), big.mark = ".")` Bewertungen → `r format(length(koerbe), big.mark = ".")` Körbe mit mindestens 2 Büchern
- Vorarbeit aus der Datenqualitäts-Übung: ISBN-Dubletten entfernt, ein Korb pro Person dedupliziert
...
itemFrequencyPlot(trans, topN = 10, cex.names = 0.6,
                  main = "Häufigste Bücher im Book-Crossing-Datensatz")
```
...
- Zum Vergleich: Bei Green Mile reichte ein einziger Band — **Popularität ist kein Muster**
...
- Ohne `unique()` je Korb und ISBN-Deduplizierung: Empfehlung = dasselbe Buch in anderer Auflage
...
- Alle folgenden Verfahren laufen auf demselben Book-Crossing-Datensatz
...
- **Wie würden Sie ihren Geschmack fassen — und mit wem vergleichen?**

## Collaborative Filtering: Wie macht Spotify das?

- Die Charts gefallen Ihnen nicht — und trotzdem trifft Spotify Ihren Geschmack. Wie?
- Nicht über den Inhalt: Der Algorithmus weiß nicht, was ein Genre ist
- Sondern über **Geschmacksnachbarn**: Menschen, die bisher ähnlich bewertet haben wie Sie
...
## Collaborative Filtering: Wie es funktioniert

- Wieder die große Tabelle: eine Zeile pro Nutzer:in, eine Spalte pro Buch, in den Zellen die Noten
...
- Drei Kandidaten treten an:
  ...
  - **IBCF** — empfiehlt über ähnliche *Bücher* („wer dieses mochte, mochte auch …")
...
- Gut ist: viel nach oben, wenig nach rechts — also eine Kurve, die über der Diagonale liegt

## Der Test im Bild

![](images/roc_cf.png)

- Beide Verfahren liegen deutlich über der Zufallslinie — besser als Raten sollte jedes Empfehlungssystem sein
...
## Clustering: Welche Lesertypen stecken in Book-Crossing?
...
## Wie würden Sie es lösen: Gefällt das Buch?
...
- Unsere Frage an Book-Crossing: **Wird diese Leserin dieses Buch mögen?**
...
- Jede Verzweigung ist eine Frage, jedes Blatt eine Vorhersage — den kann man Betroffenen zeigen
...
- Jeder Punkt ein Buch — rechts unten, allein: *Wild Animus*
...
- Jeder Punkt ein Buch, jede Linie gemeinsame Leser:innen — die Farben sind die automatisch gefundenen Gruppen
...
## Welches Verfahren für welche Frage?

Alle sieben mit Code und Ergebnissen im Notebook:
...
- **Wollen Bibliotheken Filterblasen — oder ist Serendipität ein Bibliothekswert?**
...
- Materialien: Ablaufplan, Quarto-Notebook mit allem Code
- **Kontakt:** [tom\@alby.de](mailto:tom@alby.de)
